...
class depthwise_separable_conv(nn.Module):
    ...
    def __init__(self, nin, kernels_per_layer, nout, sz, dilation=1):
        ...
        # TODO: introduce also 3D variant (read https://discuss.pytorch.org/t/3d-depthwise-separable-convolution/115591/3)
        self.depthwise = nn.Conv2d(
            nin,
            nin * kernels_per_layer,
            kernel_size=sz,
            padding=dilation * ((sz - 1) // 2),
            dilation=dilation,
            groups=nin,
        )
...
def batchconv_dsc(in_channels, out_channels, sz, conv_3D=False):
    # conv_layer = nn.Conv3d if conv_3D else nn.Conv2d
    ...
    return nn.Sequential(
        batch_norm(in_channels, eps=1e-5, momentum=0.05),
        nn.ReLU(inplace=True),
        conv_layer(in_channels, 1, out_channels, sz, dilation=1),
    )


def batchconv0_dsc(in_channels, out_channels, sz, conv_3D=False):
    # conv_layer = nn.Conv3d if conv_3D else nn.Conv2d
    conv_layer = depthwise_separable_conv
    batch_norm = nn.BatchNorm3d if conv_3D else nn.BatchNorm2d
    return nn.Sequential(
...
class resdown(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, sz, conv_3D=False, depthwise=False):
        super().__init__()
            ...
            if t == 0:  # in_channels, out_channels
                if depthwise:
                    self.conv.add_module(
                        "conv_%d" % t,
                        batchconv_dsc(in_channels, out_channels, sz, conv_3D),
                    )
                else:
                    self.conv.add_module(
                ...
                if depthwise:  # if depthwise and t % 2 == 0:
                    self.conv.add_module(
                        "conv_%d" % t,
                        batchconv_dsc(out_channels, out_channels, sz, conv_3D),
                    )
                else:
                    self.conv.add_module(
                        "conv_%d" % t,
...
class downsample(nn.Module):
    ...
    def __init__(self, nbase, sz, conv_3D=False, max_pool=True, depthwise=False):
        super().__init__()
        ...
        for n in range(len(nbase) - 1):
            self.down.add_module(
                "res_down_%d" % n,
                resdown(nbase[n], nbase[n + 1], sz, conv_3D, depthwise=depthwise),
            )
...
class batchconvstyle(nn.Module):
    ...
    def __init__(
        ...
        if depthwise:
            self.conv = batchconv_dsc(in_channels, out_channels, sz, conv_3D)
        else:
...
class resup(nn.Module):
    ...
    def __init__(
        ...
        if depthwise:
            self.conv.add_module(
                "conv_0", batchconv_dsc(in_channels, out_channels, sz, conv_3D=conv_3D)
            )
        else:
            self.conv.add_module(
        ...
        if depthwise:
            self.proj = batchconv0_dsc(in_channels, out_channels, 1, conv_3D=conv_3D)
        else:
...
class upsample(nn.Module):
    ...
    def __init__(self, nbase, sz, conv_3D=False, depthwise=False):
        super().__init__()
        ...
        for n in range(1, len(nbase)):
            self.up.add_module(
                "res_up_%d" % (n - 1),
                resup(
                    nbase[n], nbase[n - 1], nbase[-1], sz, conv_3D, depthwise=depthwise
                ),
            )
...
class CPnet(nn.Module):
    ...
    def __init__(
        ...
        self.downsample = downsample(
            nbase, sz, conv_3D=conv_3D, max_pool=max_pool, depthwise=depthwise
        )
        nbaseup = nbase[1:]
        nbaseup.append(nbaseup[-1])
        self.upsample = upsample(nbaseup, sz, conv_3D=conv_3D, depthwise=depthwise)
        self.make_style = make_style(conv_3D=conv_3D)
        if depthwise:
            self.output = batchconv_dsc(nbaseup[0], nout, 1, conv_3D=conv_3D)
        else:
