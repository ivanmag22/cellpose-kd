...
class downsample(nn.Module):
    def __init__(self, nbase, sz, conv_3D=False, max_pool=True, dilations=[1, 2]):
        ...
        for n in range(len(nbase) - 1):
            self.small.add_module(
                f"small_{n}",
                nn.Conv2d(nbase[n], nbase[n], sz, stride=2, padding=1, dilation=3),
            )
        #
    ...
    def forward(self, x):
        ...
        for n in range(len(self.down)):
            ...
            #
            y = self.small[n](xd[n - 1]) if n > 0 else x
            #
...
class resup(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, style_channels, sz, conv_3D=False):
        ...
        self.conv.add_module(
            "conv_0",
            batchconv(in_channels, out_channels, sz, conv_3D=conv_3D),  # dilation=1
        )
        self.conv.add_module(
            "conv_1",
            batchconvstyle(
        ...
        self.proj = batchconv0(
            in_channels, out_channels, 1, conv_3D=conv_3D
        )  # dilation=1
