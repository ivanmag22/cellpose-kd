...
def batchconv(in_channels, out_channels, sz, conv_3D=False, dilation=1):
    # conv_layer = nn.Conv3d if conv_3D else nn.Conv2d
    conv_layer = depthwise_separable_conv
    batch_norm = nn.BatchNorm3d if conv_3D else nn.BatchNorm2d
    return nn.Sequential(
        batch_norm(in_channels, eps=1e-5, momentum=0.05),
        nn.ReLU(inplace=True),
        conv_layer(in_channels, 1, out_channels, sz, dilation=dilation),
    )


def batchconv0(in_channels, out_channels, sz, conv_3D=False, dilation=1):
    # conv_layer = nn.Conv3d if conv_3D else nn.Conv2d
    conv_layer = depthwise_separable_conv
    batch_norm = nn.BatchNorm3d if conv_3D else nn.BatchNorm2d
    return nn.Sequential(
...
class depthwise_separable_conv(nn.Module):
    # reference: https://github.com/seungjunlee96/Depthwise-Separable-Convolution_Pytorch
    def __init__(self, nin, kernels_per_layer, nout, sz, dilation=1):
        """
        kernels_per_layer = 1: Minimal extra cost, common in lightweight architectures.
        kernels_per_layer > 1: Can improve feature extraction at the cost of increased computation and parameters
        """
...
class transp_depthwise_separable_conv(nn.Module):
    # reference: https://github.com/seungjunlee96/Depthwise-Separable-Convolution_Pytorch
    def __init__(self, nin, kernels_per_layer, nout, sz=2, s=2, p=1, o_p=1, d=2):
        """
        kernels_per_layer = 1: Minimal extra cost, common in lightweight architectures.
        kernels_per_layer > 1: Can improve feature extraction at the cost of increased computation and parameters
        """
        super(transp_depthwise_separable_conv, self).__init__()
        self.transp_depthwise = nn.ConvTranspose2d(
            nin,
            nin * kernels_per_layer,
            kernel_size=sz,
            stride=s,
...
class resdown(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, sz, conv_3D=False, dilations=[1, 2]):
        super().__init__()
        self.conv = nn.Sequential()
        self.proj = batchconv0(in_channels, out_channels, 1, conv_3D)

        for t, dilation in enumerate(dilations):
            if t == 0:  # in_channels, out_channels
                self.conv.add_module(
                    "conv_%d" % t,
                    batchconv(
                        in_channels, out_channels, sz, conv_3D, dilation=dilation
                    ),
                )
            else:  # out_channels, out_channels
                self.conv.add_module(
                    "conv_%d" % t,
                    batchconv(
                        out_channels, out_channels, sz, conv_3D, dilation=dilation
                    ),
                )
...
class downsample(nn.Module):
    def __init__(self, nbase, sz, conv_3D=False, max_pool=True, dilations=[1, 2]):
        """
        Downsample block using resdown layers.

        Args:
            nbase (list): List of channel sizes.
            sz (int): Kernel size.
            conv_3D (bool): Whether to use 3D convolutions.
            max_pool (bool): Whether to use max pooling.
            dilations (list): List of dilation rates for each `resdown`. Defaults to [1, 2].
        """
        super().__init__()
        ...
        self.maxpool = (
            ...
            if max_pool
            else (nn.AvgPool3d(2, stride=2) if conv_3D else nn.AvgPool2d(2, stride=2))
        )

        for n in range(len(nbase) - 1):
            self.down.add_module(
                f"res_down_{n}",
                resdown(nbase[n], nbase[n + 1], sz, conv_3D, dilations=dilations),
            )
...
class resup(nn.Module):
    ...
    def __init__(
        ...
        if need2upsample:
            self.conv.add_module(
                "conv_0",
                transp_depthwise_separable_conv(
                    in_channels,
                    1,
                    out_channels,
                    sz=2,
...
class upsample(nn.Module):
    ...
    def __init__(self, nbase, sz, conv_3D=False):
        ...
        for n in range(1, len(nbase)):
            ...
            self.up.add_module(
                "res_up_%d" % (n - 1),
                resup(nbase[n], nbase[n - 1], nbase[-1], sz, conv_3D, need2upsample),
            )
...
class CPnet(nn.Module):
    ...
    def __init__(
        ...
        self.downsample = downsample(
            nbase, sz, conv_3D=conv_3D, max_pool=max_pool, dilations=dilations
        )
        nbaseup = nbase[1:]
