...
class resup(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, style_channels, sz, conv_3D=False):
        ...
        # self.conv1by1 = batchconv0(out_channels, out_channels, 1, conv_3D=conv_3D)

    """
    def forward(self, x, y, style, mkldnn=False):
        x = self.proj(x) + self.conv[1](style, self.conv[0](x), y=y, mkldnn=mkldnn)
        x = x + self.conv[2](style, x, mkldnn=mkldnn)
        # x = self.conv1by1(x) + self.conv[2](style, x, mkldnn=mkldnn)
...
class upsample_compr(nn.Module):
    ...
    def __init__(self, nbase, sz, conv_3D=False):
        ...
        for n in range(1, len(nbase)):
            ...
            # 1st: F=2, S=2, P=0, d=1, P_out=0 OK
            self.up_t.add_module(
                "res_up_%dt" % (n - 1),
                nn.ConvTranspose2d(
                    in_channels=nbase[n - 1],
    ...
    def forward(self, style, xd, mkldnn=False):
        # print(f"upsample_compr: 1) x.shape={xd[-1].shape}")
        x = self.up[-1](xd[-1], xd[-1], style, mkldnn=mkldnn)
        # print(f"upsample_compr: 2) x.shape={x.shape}")
        for n in range(len(self.up) - 2, -1, -1):
            if mkldnn:
                x = self.up_t[n + 1](x.to_dense()).to_mkldnn()
                # print(f"upsample_compr (transp conv): 3) x.shape={x.shape}")
            else:
                x = self.up_t[n + 1](x)
                # print(f"upsample_compr (transp conv): 3) x.shape={x.shape}")
            x = self.up[n](x, xd[n], style, mkldnn=mkldnn)
            # print(f"upsample_compr (resup): 4) x.shape={x.shape}")
        return x


class upsample_compr1(nn.Module):
    ...
    def forward(self, style, xd, mkldnn=False):
        ...
        for n in range(len(self.up) - 2, -1, -1):
            if n % 2 != 0:
                # print(f"{n}) transposed convolution layer")
                if mkldnn:
                    x = self.up_t[n + 1](x.to_dense()).to_mkldnn()
                    # print(f"upsample_compr (transp conv): 3) x.shape={x.shape}")
                else:
                ...
                # print(f"{n}) upsample layer")
                if mkldnn:
                    x = self.upsampling(x.to_dense()).to_mkldnn()
                    # print(f"upsample_compr (upsample): 3) x.shape={x.shape}")
                else:
                    x = self.upsampling(x)
                    # print(f"upsample_compr (upsample): 3) x.shape={x.shape}")
            x = self.up[n](x, xd[n], style, mkldnn=mkldnn)
...
class CPnet(nn.Module):
    ...
    def __init__(
        ...
        # self.upsample = upsample_compr(nbaseup, sz, conv_3D=conv_3D)
        self.upsample = upsample_compr1(nbaseup, sz, conv_3D=conv_3D)
