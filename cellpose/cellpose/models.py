...
from .resnet_torchv2 import CPnet as CPnet_depth
from .dilated_resnet import CPnet as CPnet_dilation
from .upsample_resnet import CPnet as CPnet_upsample
from .downup_resnet import CPnet as CPnet_downup
from .fastcp_resnet_torch import CPnet as FastCPnet
from .downup_resnetv2 import CPnet as CPnet_depthdownup
...
class CellposeModel:
    ...
    def __init__(
        ...
        if backbone == "default":
            if dilation and upsample_compr:
                if depthwise:
                    self.net = CPnet_depthdownup(
                        self.nbase,
                        self.nclasses,
                        sz=3,
                        mkldnn=self.mkldnn,
                        max_pool=pooling,
                        diam_mean=self.diam_mean,  # dilations=[1,2]
                    ).to(self.device)
                else:
                    self.net = CPnet_downup(
                        self.nbase,
                        self.nclasses,
                        sz=3,
                        mkldnn=self.mkldnn,
    ...
    def eval(
        ...
        if isinstance(x, list) or x.squeeze().ndim == 5:
            ...
            for i in iterator:  # it iterates over images
                tic = time.time()
            ...
            # if x is an array (2D/3D/4D image)
            # reshape image
            ...
            ax.set_axis_off()
            fig.add_axes(ax)
            ax.imshow(cellprob[0] > 0.5, cmap="gray")
            ax.set_title("Cellprob binary mask")
            # plt.savefig("cell_prob_mask.png")
            plt.show()

            # plt.imshow(cellprob[0] > 0.5, cmap="gray")
            # plt.title("Cellprob binary mask")
            # plt.savefig("cell_prob_mask.png")
            # plt.show()

            # fig, ax = plt.subplots()
            # pos = ax.imshow(dP[0][0])
            ...
            ax.set_axis_off()
            fig.add_axes(ax)
            ax.imshow(dP[0][0], cmap="bwr")
            ax.set_title("Vertical flow")
            # plt.savefig("vertical_flow.png")
            plt.show()

            # plt.imshow(dP[0][0], cmap="bwr")
            # plt.title("Vertical flow")
            # plt.savefig("Vertical_flow.png")
            # plt.show()

            # fig, ax = plt.subplots()
            # pos = ax.imshow(dP[1][0])
            # fig.colorbar(pos, ax=ax)
            fig = plt.figure(frameon=False)
            # w, h = pred.shape
            # fig.set_size_inches(w, h)
            ...
            ax.set_axis_off()
            fig.add_axes(ax)
            ax.imshow(dP[1][0], cmap="bwr")
            ax.set_title("Horizontal flow")
            # plt.savefig("horizontal_flow.png")
            plt.show()

            # plt.imshow(dP[1][0], cmap="bwr")
            ...
            if compute_masks:
                ...
                masks = self._compute_masks(
                    ...
                    stitch_threshold=stitch_threshold,
                    do_3D=do_3D,
                    down_size=down_size,  # in order to have a faster mask computation
                )
            else:
                masks = np.zeros(0)  # pass back zeros if not compute_masks
            end = time.time()
            print(f"Elapsed time for mask computation: {end-start:.2f} s")
            """
            import matplotlib.pyplot as plt
    ...
    def _compute_masks(
        ...
        if do_3D:
            ...
            # MASK COMPRESSION
            #
            whole_down_dp = dP  # (3, z, y, x)
            whole_down_cellprob = cellprob  # (z, y, x)
            #

            if down_size > 1:
                # downsample
                real_shape = cellprob.shape[-3:]
                # print(real_shape)
                tmp = np.zeros(
                    (
                        3,
                ...
                for i in range(real_shape[0]):
                    # flows
                    tmp[0, i, :, :] = transforms.resize_image(
                        whole_down_dp[0, i],
                        real_shape[1] // down_size,
                        real_shape[2] // down_size,
                        interpolation=cv2.INTER_LINEAR,
                    )
                    tmp[1, i, :, :] = transforms.resize_image(
                        whole_down_dp[1, i],
                        real_shape[1] // down_size,
                        real_shape[2] // down_size,
                        interpolation=cv2.INTER_LINEAR,
                    )
                    tmp[2, i, :, :] = transforms.resize_image(
                        whole_down_dp[2, i],
                        real_shape[1] // down_size,
                        real_shape[2] // down_size,
                        interpolation=cv2.INTER_LINEAR,
                    ...
                    dp.append(tmp[:, i])
                    # cellprob
                    tmp_c = transforms.resize_image(
                        whole_down_cellprob[i],
                        real_shape[1] // down_size,
                        real_shape[2] // down_size,
                        interpolation=cv2.INTER_LINEAR,
                    )
                    cprob.append(tmp_c)
                whole_down_dp = np.array(dp).transpose((1, 0, 2, 3))
                del dp
                whole_down_cellprob = np.array(cprob)
                del cprob
                #
            outputs = dynamics.resize_and_compute_masks(
                whole_down_dp,  # dP[:, i],
                whole_down_cellprob,  # cellprob[i],
                niter=niter,
                cellprob_threshold=cellprob_threshold,
                flow_threshold=flow_threshold,
                interp=interp,
                do_3D=do_3D,
                resize=(
                    real_shape if down_size > 1 else None
                ),  # upsample if down_size > 1 else same sizes
                min_size=min_size,
                max_size_fraction=max_size_fraction,
                device=self.device,
            )
            ...
            for i in iterator:
                ...
                if down_size > 1:
                    # downsample
                    real_shape = cellprob.shape[-2:]
                    # print(real_shape)
                    tmp = np.zeros(
                        (2, real_shape[0] // down_size, real_shape[1] // down_size),
                        np.float32,
                    )
                    tmp[0, :, :] = transforms.resize_image(
                        whole_down_dp[0],
                        real_shape[0] // down_size,
                        real_shape[1] // down_size,
                        interpolation=cv2.INTER_LINEAR,
                    )
                    tmp[1, :, :] = transforms.resize_image(
                        whole_down_dp[1],
                        real_shape[0] // down_size,
                        real_shape[1] // down_size,
                        interpolation=cv2.INTER_LINEAR,
                    ...
                    whole_down_dp = tmp
                    del tmp
                    whole_down_cellprob = transforms.resize_image(
                        whole_down_cellprob,
                        real_shape[0] // down_size,
                        real_shape[1] // down_size,
                        interpolation=cv2.INTER_LINEAR,
                ...
                outputs = dynamics.resize_and_compute_masks(
                    ...
                    interp=interp,
                    resize=(
                        real_shape if down_size > 1 else resize
                    ),  # upsample if down_size > 1 else same sizes
                    min_size=min_size0,
                    max_size_fraction=max_size_fraction,
                    device=self.device,
