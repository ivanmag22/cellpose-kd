...
def run_net(
    ...
    styles = np.zeros((Lz, net.nbase[-1]), "float32")
    ...
    for k in ziterator:
        # tq = tqdm(total=len(ziterator))
        # tq.set_description('progress: %d/%d' % (k, len(ziterator)))
        ...
        stylea = np.zeros((IMGa.shape[0], net.nbase[-1]), "float32")
