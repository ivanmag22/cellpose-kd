...
def masks_to_flows_cpu(masks, device=None, niter=None):
    ...
    for i in prange(len(slices)):
        ...
        if si is not None:
            ...
            dynamics_logger.info(dy)
            dynamics_logger.info(dx)
...
def remove_bad_flow_masks(masks, flows, threshold=0.4, device=torch.device("cpu")):
    ...
    # print(badi)
    # print(np.isin(masks, badi).shape)
    masks[np.isin(masks, badi)] = 0
    return masks


def remove_bad_flow_masks_torch(
    masks, flows, threshold=0.4, device=torch.device("cpu")
):
    """Remove masks which have inconsistent flows.
    ...
    """
    device0 = device
    if torch.numel(masks) > 10000 * 10000 and (
        ...
        total_mem, free_mem = mem_info()
        if torch.numel(masks) * 32 > free_mem:
            dynamics_logger.warning(
                "WARNING: image is very large, not using gpu to compute flows from masks for QC step flow_threshold"
            )
            dynamics_logger.info("turn off QC step with flow_threshold=0 if too slow")
    ...
    merrors, _ = metrics.flow_error(
        masks.detach().cpu().numpy(), flows.detach().cpu().numpy(), device0
    )
    badi = 1 + (merrors > threshold).nonzero()[0]
    badi = torch.from_numpy(badi)
...
def get_masks_torch_t(pt, inds, shape0, rpad=20, max_size_fraction=0.4):
    """Create masks using pixel convergence after running dynamics.
    ...
    M1 = M1.cpu()
    w, h = shape0
    # dtype = torch.uint16 if n_seeds < 2**16 else torch.uint32
    M0 = torch.zeros(w, h, dtype=dtype)
    M0[inds] = M1
    M0_np = M0.detach().cpu().numpy()
    uniq, counts = fastremap.unique(M0_np, return_counts=True)
    big = np.prod(shape0) * max_size_fraction
    bigc = uniq[counts > big]
    if len(bigc) > 0 and (len(bigc) > 1 or bigc[0] != 0):
        M0_np = fastremap.mask(M0_np, bigc)
    fastremap.renumber(
        M0_np, in_place=True
    )  # convenient to guarantee non-skipped labels
    M0 = torch.from_numpy(M0_np.reshape(tuple(shape0)))
    return M0.detach()  # .cpu().numpy()
    #
    M1 = M1.cpu().numpy()
...
def compute_masks(
    ...
    if (
        ...
        plt.imshow(mask, cmap="CMRmap")
        plt.title("Dynamics n.1")
        plt.show()
        """
...
def compute_masks_torch(
    ...
    if (
        ...
        inds = torch.nonzero(cellprob > cellprob_threshold, as_tuple=True)

        if len(inds[0]) == 0:
            dynamics_logger.info("No cell pixels found.")
            shape = cellprob.shape
            mask = torch.zeros(shape, dtype=torch.uint16)
            return mask
        import time

        p_final = follow_flows(
            dP.detach().cpu().numpy()
            * (cellprob.detach().cpu().numpy() > cellprob_threshold)
            / 5.0,
            inds=tuple(
                (inds[0].detach().cpu().numpy(), inds[1].detach().cpu().numpy())
            ),
            niter=niter,
            interp=interp,
            device=device,
        ...
        if not do_3D:
            if mask.max() > 0 and flow_threshold is not None and flow_threshold > 0:
                # make sure labels are unique at output of get_masks
                mask = remove_bad_flow_masks_torch(
                    mask, dP, threshold=flow_threshold, device=device
                )

        if mask.max() < 2**16 and mask.dtype != torch.uint16:
            mask = mask.to(dtype=torch.uint16)
