...
from . import fastcp_resnet_torch as resnet_torch
from .models_fastcp import models_logger
...
class UnetModel:
    ...
    def train(
        ...
        train_data, train_labels, test_data, test_labels, run_test = (
    ...
    def _train_net(
        ...
        if isinstance(learning_rate, (list, np.ndarray)):
            ...
            if SGD:
                ...
                if self.n_epochs > 50:
                    LR = np.append(
                        LR, self.learning_rate_const * np.ones(self.n_epochs - 60)
                    )
                    for i in range(5):
        ...
        self.batch_size = batch_size
        self._set_optimizer(
            self.learning_rate[start_epoch], momentum, weight_decay, SGD
        )
        self._set_criterion()
        ...
        for iepoch in range(start_epoch + 1, self.n_epochs):
            ...
            if test_data is not None:
                ...
                core_logger.info(
                    "Epoch %d, Time %4.1fs, Loss %2.4f, Loss Test %2.4f, LR %2.4f. DICE %2.4f, IOU %2.4f"
                    % (
                        iepoch,
                        time.time() - tic,
                        lavg,
                ...
                with open(log_path + "log_test.txt", "a") as f:
                    f.write(
                        "Epoch : {}, loss: {:4f}, loss test: {:4f}, LR {:5f}, DICE {:4f}, IOU {:4f}\n".format(
                            iepoch + 1,
                            lavg,
                            lavgt / nsum,
                            self.learning_rate[iepoch],
                ...
                if save_each:  # separate files as model progresses
                    if model_name is None:
                        ...
                        file_name = os.path.join(
                            self.net_type + "epoch_" + str(iepoch) + ".pth"
                        )
                    else:
                    ...
                    if model_name is None:
                        ...
                        file_name = os.path.join(
                            self.net_type + "_epoch_" + str(iepoch) + ".pth"
                        )
                    else:
                        file_name = "{}_{}.pth".format(
