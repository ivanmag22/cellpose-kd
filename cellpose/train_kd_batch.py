...
def preprocess_and_infer_tensor(
    ...
    for k in range(niter):
        ...
        for i, b in enumerate(inds):
            ...
            imgb = resize_t(torch.from_numpy(img[b]))
            pads = (xpad1, xpad2, ypad1, ypad2, 0, 0)
            imgb = F.pad(imgb, pads, mode="constant")
...
def val_iou(
    ...
    for i, data in enumerate(dataloader):
        ...
        if display:
            for i, x in enumerate(t_cellprob):
                plt.imshow(x > 0.5, cmap="gray")  # plt.imshow(x > 0.5, cmap="gray")
...
def train_kd(
    ...
    writer = SummaryWriter(
        comment=f"D{diameter}_LR{learning_rate:.2e}_BS{bs}_EP{n_epochs}"
    )
    ...
    for epoch in range(epoch_start_i, n_epochs):
        ...
        for i, train_batch in enumerate(
            ...
            if display:
                if student_cellprob.ndim == 2:
                    ...
                    for i, x in enumerate(student_cellprob):
                        ...
                        plt.imshow(x.detach().cpu().numpy() > 0.5, cmap="gray")
            ...
            if display:
                if teacher_cellprob.ndim == 2:
                    tiff.imshow(
                        teacher_cellprob.detach().cpu().numpy() > 0.5, cmap="gray"
                    )
...
def main(args):
    ...
    if s_fastcellpose:
        ...
        s_model = FastCellpose(
            gpu=True if device.type == "cuda" else False,
            device=device,
            pretrained_model=os.path.abspath(inference_model_name),
            net_avg=False,
        )
    else:
        s_model = Cellpose(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="new",
        help="Loss function: 'default' from train.py (lambda=5), 'new' for train.py loss function with lambda=1, 'cellprob' for train.py loss function with lambda=1 and high weight to cellprob contribution, 'bce' using Binary Cross Entropy loss. Default: 'new'",
    )
    parser.add_argument(
        "--saved_model_dir",
        type=str,
